...
foreach ($f in $files) {
    ...
    $result = & clang-tidy @args $f.FullName -- -std=c++23 -I"$PWD\src" -I"$PWD\include" -I"$PWD" -I"$PWD\vcpkg_installed\x64-windows\include" -I"$PWD\build\vcpkg_installed\x64-windows-static\include" -DUNICODE -D_UNICODE -D_CRT_SECURE_NO_WARNINGS --target=x86_64-pc-windows-msvc -fms-compatibility -fms-extensions 2>&1
